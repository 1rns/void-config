...
local user_opts = {
    ...
    oscboxheight = 78,
    oscboxwidth = 900,
    seekbarstyle = "bar",       -- bar, knob
    pbseekbarheight = 10,       -- playback seekbar, height of the box and handle
    pbseekbarhandlesize = 10,
    volseekbar = true,          -- volume seekbar visibility, true/nil
    volseekbarstyle = "volknob",   -- volknob, bar
    volboxheight = 3.5,         -- volume seekbar height
    volboxwidth = 80,
    volhandlesize = 14,
...
local osc_styles = {
    smallButtonsLlabel = "{\\fs13\\fn" .. mp.get_property("options/osd-font") .. "}",

    elementDown = "{\\1c&H9E9E9E}",
    box = "{\\rDefault\\blur0\\bord0\\1c&H101010\\3c&HEFEFEF}",
    seekbar = "{\\1c&HEFEFEF}",

    --buttons
    buttonsLeft = "{\\blur0\\bord0\\1c&HEFEFEF\\3c&HEFEFEF\\fs17.5\\fnoscc}",
    buttonsRight = "{\\blur0\\bord0\\1c&HEFEFEF\\3c&HEFEFEF\\fs17.5\\fnoscc}",

    --times
    timecodesBar = "{\\blur0\\bord0\\1c&HEFEFEF\\3c&HEFEFEF\\fs17.5}",
    timePosBar = "{\\blur2.4\\bord".. user_opts.tooltipborder .."\\1c&HEFEFEF\\3c&H000000\\fs" .. user_opts.tooltipfontsize .."}",
}
...
function get_slider_value_at(element, glob_pos)
    ...
    local val = scale_value(
        ...
        element.slider.min.value, element.slider.max.value, glob_pos)

    return limit_range(
        element.slider.min.value, element.slider.max.value, val)
...
function bar_layout(direction)
    local osc_geo = {
        ...
        y,
        an = 4,
        w = user_opts.oscboxwidth,
        h = user_opts.oscboxheight,
    }


    local btnw_left = string.match(osc_styles.buttonsLeft, "fs(.%d)")
